...
}CustomerNode;				//队列的数据元素类型 

typedef CustomerNode QElemType;
...
Status ListInsert_L(LinkList &L,int i,ElemType e){
    ...
    //正确j的可能值：0，(i-1),  
    if(!q||j>i-1)                        //1.q为NULL(i大于表长+1)  2.i不能小于1
        return ERROR;
    LNode * s;
...
Status ListDelete_L(LinkList &L,int i,ElemType &e){
    ...
    if(!(q->next)||j>i-1)                //1.q->next不能为NULL(i位置不存在) 
        return ERROR;
    LNode * tem=q->next;                //保存 将被删除元素的坐在位置
    e=tem->data;                    //将被删除的值保存在e中
    
    q->next=tem->next;                //删除元素
...
void CustomerArrived(){
    ...
    int depT=en.OccurTime+durTime;					//当前客户离开的时间 
    int arrT=en.OccurTime+interTime;				//下一个客户到达时间 
